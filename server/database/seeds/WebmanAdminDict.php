...
class WebmanAdminDict extends AbstractSeed
{
  ...
  public function run(): void
  {
    if (function_exists('config') && config('database.default')) {
      // 第三方平台字典
      if (Db::schema()->hasTable('wa_options')) {
        if (!OptionModel::where('name', 'dict_open_account_types')->first()) {
        ...
        if (!OptionModel::where('name', 'dict_name')->first()) {
          OptionModel::unguarded(function () {
            $value = [
              ["value" => "status", "name" => "状态"],
              ["value" => "sex", "name" => "性别"],
              ["value" => "upload", "name" => "附件分类"],
            ];
            OptionModel::create([
              'name'  => 'dict_name',
              'value' => json_encode($value, JSON_UNESCAPED_UNICODE),
